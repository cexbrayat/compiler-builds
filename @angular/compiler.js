...
class FromJsonDeserializer extends ValueTransformer {
    ...
    deserialize(libraryFileName, json) {
        ...
        data.symbols.forEach((serializedSymbol) => {
            const symbol = this.symbolCache.get(this.summaryResolver.fromSummaryFileName(serializedSymbol.filePath, libraryFileName), serializedSymbol.name);
...
class AotCompiler {
    ...
    _compileImplFile(srcFileUrl, ngModuleByPipeOrDirective, directives, pipes, ngModules, injectables) {
        ...
        directives.forEach((dirType) => {
            ...
            // Note: compMeta is a component and therefore template is non null.
            compMeta.template.externalStylesheets.forEach((stylesheetMeta) => {
                generatedFiles.push(this._codegenStyles(stylesheetMeta.moduleUrl, compMeta, stylesheetMeta, fileSuffix));
            });
            // compile components
    ...
    _codegenStyles(srcFileUrl, compMeta, stylesheetMetadata, fileSuffix) {
        const outputCtx = this._createOutputContext(_stylesModuleUrl(stylesheetMetadata.moduleUrl, this._styleCompiler.needsStyleShim(compMeta), fileSuffix));
        const compiledStylesheet = this._styleCompiler.compileStyles(outputCtx, compMeta, stylesheetMetadata);
...
class StaticReflector {
    ...
    simplify(context, value) {
        ...
        const recordedSimplifyInContext = (context, value) => {
            try {
...
class JitCompiler {
    ...
    _interpretOrJit(sourceUrl$$1, statements) {
